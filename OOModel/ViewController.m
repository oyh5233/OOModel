...
- (void)viewDidLoad {
    [super viewDidLoad];
    [self performSelector:@selector(createAModel) onThread:self.subThread withObject:nil waitUntilDone:NO modes:@[NSRunLoopCommonModes]];
...
- (void)startSubThread{
    @autoreleasepool {
        [[NSThread currentThread]setName:@"Sub Thread"];
...
- (NSThread*)subThread{
    if (!_subThread) {
        _subThread=[[NSThread alloc]initWithTarget:self selector:@selector(startSubThread) object:nil];
        [_subThread start];
    }
    return _subThread;
}
